...
module.exports = class extends ModuleBundler {
	...
	/**
	 * Used by the distribution to inject code at the beginning of the bundle.
	...
	_update(hmr) {
		...
		const pdependencies = this.distribution.distribution.local && this.children.get('dependencies.packages').child;
		...
		const transversal = !!this.distribution.application.bundles.get(distribution.bundle.type).transversal;
