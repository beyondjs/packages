...
**Updates.** The Dev Server applies nothing to a page: the runtime does. When the runtime the artifacts are assembled against is a workspace package that also publishes a `main` module, the document imports that coordinator and calls `local.register({ origin, options })` with the base of the preview before it imports the application, and `updates.runtime` names it. This is a provisional convention, like the `/u/` route. When the runtime has no coordinator in the workspace, which is the case of the published Kernel, `updates.reason` says that nothing applies updates to the running page: new code then appears when the document is loaded again, which is a reload and is never reported as an update.
...
- A preview applies updates only when the workspace contains a development runtime with a coordinator. The development runtime is not distributed, so a project that uses the published Kernel is rebuilt on save and shown on reload. Styles and Widgets have no artifact, so nothing updates them.
