...
It lazily converts map formats and appends inline maps when requested. Its hash covers code, not map content. An output without code (a stylesheet-only module, a failed processing) answers `undefined` for its code and hash, and a map that is not JSON is no map: it never throws, because it is read while a conditional processes.

A processing that throws never leaves a request waiting. The dynamic processor settles the readiness of an object only when a processing returns, so [BaseConditional](../modules/module/main/conditionals/conditional/index.ts) wraps the `_process` of every conditional: a throw or a rejection becomes `PROCESSING_FAILED` in its `errors`, it is not `valid`, and the next processing that succeeds reports a change. [ConditionalProcessor](../modules/sdk/conditional/processor/index.ts) does the same for the `_build` of a processor (`PROCESSOR_FAILED`), which the conditional reads with the other diagnostics of its processors. [The processing validation](../tests/processing/README.md) executes both with a defective bundler. It does not schedule builds, aggregate diagnostics, persist an artifact, register a URL or install styles. [IssuesOutput](../modules/sdk/conditional/processor/outputs/output/issues/index.ts) accepts `push('errors' | 'warnings', {code, message, position?})`. Per-source issues are separate from the conditional's own diagnostics.
...
The scope that joins a component's code to its styles — the `data-v-…` attribute of a Vue component, the `svelte-…` class of a Svelte one — is derived by [Scope](../modules/sdk/conditional/processor/scope.ts) from the identity of the package the component belongs to and the component's path in that package, never from where the files are. `Package.identity` is what a reader of the package gave it: the node key and integrity of a pinned package (the preparation of a published package sets it), and otherwise its name and version, which a workspace makes unique. Components at the same path in different packages or modules therefore have different scopes, one component has the same scope wherever its sources were extracted, and the scope is the same on every conditional and across edits, so a server render and the client that hydrates it agree and a mounted view keeps matching its updated stylesheet. [The identity validation](../tests/identity/README.md) executes it.

Errors keep the positions of the authored file (`STYLE_ERROR`, `TAILWIND_ERROR`, `VUE_PARSE_ERROR`, `VUE_SCRIPT_ERROR`, `VUE_TEMPLATE_ERROR`, `VUE_STYLE_ERROR`, `SVELTE_ERROR`), and a module with an invalid stylesheet or component is not published. The stylesheets of a module are its style sources, not imports of its code: `import './x.css'` belongs to the esbuild packaging mode, whose bundler follows the imports of the entry point. The stylesheet of another public module is selected by specifier with `.css` (below).
...
### Selecting the stylesheet of a public module

An import selects the JavaScript output of a public module; an explicit `.css` selects its stylesheet (`import '@scope/ui/theme.css'` is the stylesheet of `./theme`, or of the literal subpath `./theme.css` when the package publishes one). The selection is a **style relation**, never an import of code, in every route:

- The `ts` bundler: the [analyzer](../modules/bundlers/ts/processors/ts/analyzer.ts) keeps a `.css` specifier out of the code dependencies, and the [Assembler](../modules/sdk/conditional/esm/assembler.ts) lists it under `stylesheets` in the bundle specification and in `IESMArtifact.stylesheets` instead of importing it; the creator's `require` of it answers an empty namespace. The artifact passes `import.meta.resolve` to the runtime, which resolves those specifiers where the page resolves modules and adopts the sheets with the styles of the module (`bundle.stylesheets`, collected by `DependenciesStyles` before the module's own sheet). `Toolchain.COMPOSITION` is `3` since this change.
- The esbuild packaging mode: the [Boundary](../modules/bundlers/esbuild/processors/bundle/boundary.ts) removes any bare `.css` specifier from the code and records it as a `style` reference; a relative import of the stylesheet another public module of the package is published from is the same selection. `Packaged` reports them as `IESMArtifact.stylesheets`, and the development delivery as `IDelivered.stylesheets`, for installed packages too.
- A stylesheet binds no value: a default or named import of it, or `with { type: 'css' }`, is `STYLE_BINDING_UNSUPPORTED`.

The development selection ([Selection](../modules/workspace/selection.ts)) resolves a selector the same way and reports the selected `output`; two different modules for one selector are `OUTPUT_AMBIGUOUS`, and `.js` of a style module is `OUTPUT_NOT_FOUND`. The same rule for published packages, with its diagnostics, is in [the outputs guide](cdn-outputs.md#output-selection).
...
A style module (`"./theme": "./theme.css"`) bundles to a stylesheet and no code: its conditional is valid with `styles` and no `output`. [Compilation](../modules/artifacts/compilation.ts) then holds the stylesheet in `styles` and is not `valid`, with `OUTPUT_NOT_FOUND`, because there is no code to publish; `Resources.styles` serves the stylesheet, `Delivery.module` answers the code as `OUTPUT_NOT_AVAILABLE` with the hash of the sheet in `failure.styles`, a development build lists the module `valid` with `styles` and no `hash`, and `Artifacts` writes the stylesheet alone. A failure while reading the outputs of a bundle is a diagnostic of the module, never an exception: a processing that throws leaves its readiness unsettled, and a request waiting on it would never answer.

The development maps of this mode name every source by its absolute path ([Located](../modules/bundlers/esbuild/processors/bundle/located.ts)), without `sourceRoot`, with the content of every source or of none, as the inline-map rule of the compiled-module contract requires; an input that is not a file (the adapted entry point, a module a namespace of the boundary stands for) is named under `~generated/` of the package root. A written artifact names them relative to itself (`Files.portable`), and a published output under `beyond://` (the generation's `Origins`).
