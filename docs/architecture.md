...
The implementation includes source collections, delegated processing, ESM assembly, maps, diagnostics and Delivery-backed development integration. The esbuild packaging path produces CSS/maps/assets and development delivery consumes them. SDK stylesheet aggregation, runtime style integration and semantic TypeScript diagnostics remain separate gaps. This guide describes existing APIs and limitations; [development](development.md) defines the intended service and widget acceptance criteria, and [programming](programming.md) explains authoring conventions.
...
The types/CSS containers provide an API slot, not automatic aggregation: `ESMConditional` collects `outputs.ims`, not `outputs.css` or `outputs.types`. The TS processor fills IMS; its tsc sibling imports a missing `./exports` and does not implement active semantic compilation/declaration emission. The esbuild processor instead exposes its own `bundle` result (it ignores the standard outputs argument); `Packaged` maps its CSS/map to `conditional.styles`, which `Artifacts.Resources.styles` consumes. CSS compilation/delivery therefore exists without a common SDK CSS aggregation path. Runtime registration, dependency loading, Shadow DOM adoption and CSS HMR are separate responsibilities.
