...
| `GET /resolution.json?target=&format=`, `GET /importmap.json?target=&format=` | The resolution documents of the contract, computed from the workspace on every request, `no-store`: every public module of the workspace and of the supplied packages that declares the target, and for browsers the installed packages and stylesheets their builds reach, resolved from the package that imports them, with a scope for an importer that reaches another version. Without a query, the Node ES module document of the session. Installed packages are compiled for browsers only, so the node document lists none: a Node consumer resolves them from its installation |
