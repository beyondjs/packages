...
| `compiler` input of the key | `esbuild`, its version and its options, and `Toolchain.SHARING` for an ordinary npm package | The specifier of the bundler, and `Toolchain.COMPOSITION` |
...
A composed module is compiled from a `Package` read from the extracted root in its real location, with the identity of its node: the node key (`origin:name@version`) and the integrity of the package. The identifiers that join the code of a component to its stylesheet are derived from that identity and the component's path in the package ([Scope](../modules/sdk/conditional/processor/scope.ts)): a Vue component's scope id and a Svelte component's CSS class. So:

- the same pinned source yields the same identifiers wherever it is extracted, and a module and its stylesheet generated in two units, each in a scratch directory of its own, still select each other;
- components at the same path in different modules or packages, and the same package name, version and path from different origins, have identifiers of their own, so their styles never apply to each other inside one shadow root;
- the conditions are not part of the identity, so a server render and the client that hydrates it agree.

Two sources of one name and version in **one** application are still not delivered: the runtime registers a loaded module by `name@version/subpath` (the vspecifier), so an application pins each name and version from one origin, which CDN enforces (`UNSUPPORTED_INPUT`). Composed diagnostics name the file in the package, never where it was extracted.

Executed by [tests/identity](../tests/identity/README.md) (6 checks), which fail against the revision before this change: a composed module differed when its root was reached through a link (its map named its own sources `~outside/<name>`), the Svelte class of a stylesheet generated on its own was not the one its code wrote, and three `view.vue` components of a widget, another module and another package shared one scope.

`Toolchain.SHARING` is the same kind of revision for the outputs of an ordinary npm package: whether its subpaths are delivered on their own or as a carrier and its facades changes what an output contains without changing any other input of its key. It was introduced with the carrier-and-facade delivery, so every npm output key changed once then, and an output stored before it is never answered for a key it no longer satisfies; raise it whenever the plan or the union it compiles changes. The keys of source and distribution forms do not include it and did not change.
...
- A compiler whose process ends is reported as `COMPILER_UNAVAILABLE` rather than as a module that does not compile, and is released so that the next unit has a compiler again. [The compiler validation](../tests/compiler/README.md) executes it with a controlled termination and, [under a real memory ceiling](../tests/compiler/README.md#under-a-real-memory-ceiling), with the kernel's memory controller ending the compiler. A source large enough to exhaust the heap of the process that reads it ends that process instead, which is not this failure.
- The public subpaths of an ordinary npm package that share internal files **are** delivered as a carrier and its facades, the same way the development delivery does it: [`Sharing`](../modules/analysis/pinned/sharing.ts) belongs to `@beyond-js/packages/analysis`, so the inventory and the generation reach the same roles from the same inputs without recording them, and `artifacts` reads it from there. A package whose root reaches the entry point of another of its public subpaths, such as `svelte`, therefore yields one copy of that state, and a component compiled against one runs on the same runtime as a view mounted from the other. A subpath whose API the union cannot list, and a package that shares internal files without ever reaching the entry point of the subpath that owns them, keep their copies; [the sharing validation](../tests/sharing/README.md) is where each of those is checked.
