...
const shared = {
	'shared/value/module.json': node,
	'shared/package.json': manifest('@case/shared', { exports: { './value': './value/index.ts' } }),
	'shared/value/index.ts': `export const value = 'shared value';\n`
};

const app = dependencies => ({
	'app/main/module.json': node,
	'app/package.json': manifest('@case/app', { exports: { './main': './main/index.ts' }, dependencies }),
	'app/main/index.ts': `import { value } from '@case/shared/value';\nexport const main = () => 'app: ' + value;\n`
});
...
export async function defects() {
	...
	await step('dependency error: the dependent artifact is neither written nor mapped', async () => {
		const fixture = await Fixture.create('undeclared', {
	...
	await step('failed rebuild: the previous artifact files do not remain reachable', async () => {
		const fixture = await Fixture.create('stale', {
			'beyond.json': { packages: ['shared', 'app'] },
			...shared,
			...app({ '@case/shared': '1.0.0' })
		});
		...
		try {
			...
			await fixture.write(app({ '@case/shared': '1.0.0' }));
			await build.run();
			assert.deepEqual(build.codes(), []);
			assert.deepEqual((await build.execute('@case/app/main')).values, { main: 'app: shared value' });
